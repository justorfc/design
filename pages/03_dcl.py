...
def validar_cuadro_latino(df_trat: pd.DataFrame, tratamientos: List[str]) -> Tuple[bool, List[str]]:
    ...
    for idx in df_trat.index:
        ...
        if fila != conjunto_esperado:
            errores.append(f"La fila '{idx}' no contiene exactamente una vez cada tratamiento.")

    for col in df_trat.columns:
        columna = sorted(df_trat[col].astype(str).tolist())
        if columna != conjunto_esperado:
            errores.append(f"La columna '{col}' no contiene exactamente una vez cada tratamiento.")
...
def expresion_suma_cuadrados(valores: List[float]) -> str:
    return " + ".join(
        [f"{v:.4f}²" if isinstance(v, float) and not float(v).is_integer() else f"{int(v)}²" for v in valores]
    )
...
if calcular:
    ...
    st.dataframe(tabla_anova, width="stretch")
    ...
    st.dataframe(df_prom, width="stretch")
